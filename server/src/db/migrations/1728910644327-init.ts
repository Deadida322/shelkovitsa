...
export class Init1728910644327 implements MigrationInterface {
    name = 'Init1728910644327'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE "product_article" ("created_at" TIMESTAMP NOT NULL DEFAULT ('now'::text)::timestamp(6) with time zone, "updated_at" TIMESTAMP NOT NULL DEFAULT ('now'::text)::timestamp(6) with time zone, "is_deleted" boolean NOT NULL DEFAULT false, "id" SERIAL NOT NULL, "name" character varying, "description" character varying, "logo" character varying, "article" character varying NOT NULL, "price" numeric NOT NULL, "productSubcategoryId" integer, CONSTRAINT "UQ_ca77e6edcf6aa36792a69e69b8f" UNIQUE ("article"), CONSTRAINT "PK_47a9fc181d48431d856e1db3a0b" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TABLE "product" ("created_at" TIMESTAMP NOT NULL DEFAULT ('now'::text)::timestamp(6) with time zone, "updated_at" TIMESTAMP NOT NULL DEFAULT ('now'::text)::timestamp(6) with time zone, "is_deleted" boolean NOT NULL DEFAULT false, "id" SERIAL NOT NULL, "amount" integer NOT NULL DEFAULT '0', "productArticleId" integer, "productSizeId" integer, "productColorId" integer, CONSTRAINT "PK_bebc9158e480b949565b4dc7a82" PRIMARY KEY ("id"))`);
        ...
        await queryRunner.query(`ALTER TABLE "product_file" ADD CONSTRAINT "FK_8908c9d576061144b562635769f" FOREIGN KEY ("productId") REFERENCES "product_article"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`);
